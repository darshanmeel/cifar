...
dgts_data = pd.read_csv("cifar_test.csv",index_col=0)

dt = np.array(dgts_data)
print dt.shape


dgts_lbl = pd.read_csv("cifar_test_l.csv",index_col=0)
...
for i in range(clstrs):
    ...
    mdl = etc(n_estimators=5000,criterion='entropy',oob_score=True,bootstrap=True,min_samples_split=10)
    #mdl = SVC(C=10000,gamma=0.00001,kernel='rbf')
    mdl = knn(n_neighbors=5)
    #mdl = mn()
    td = train_dt[t_idx]
    tc = train_lbl[t_idx]
    #print tc
...
for i in range(clstrs):
    ...
    print i
    '''
    tsc_pd = pd.DataFrame(tsc)
    tsc_pd_grpd = tsc_pd.groupby(as_index=False)
    print tsc_pd_grpd.count()
    print ghy
    '''
    #print tsc
    mdl = mdls[i]
